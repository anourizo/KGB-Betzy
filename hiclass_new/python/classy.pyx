...
cdef class Class:
    ...
    def rs_at_z(self, z):
        """
        r_s(z)
        Return the comoving sound horizon at z (exactly, the quantity defined by
        Class as index_bg_rs in the background module)
        Parameters
        ----------
        z : float
                Desired redshift
        ...
        rs = pvecback[self.ba.index_bg_rs]
    ...
    def M2_smg(self, z):
        """
        Effective Planck mass square (M^2_*) (z)

        Return the Effective Planck Mass square (exactly, the quantity defined by Class as index_bg_M2_smg
        in the background module)
        ...
        M2_smg = pvecback[self.ba.index_bg_M2_smg]
    ...
    def alphaM(self, z):
        """
        Planck mass run rate (alphaM) (z)

        Return the Planck Mass run rate (exactly, the quantity defined by Class as index_bg_mpl_running_smg
        in the background module)
        ...
        alphaM = pvecback[self.ba.index_bg_mpl_running_smg]
    ...
    def z_optical_depth_unity(self):
        """Return the redshift at which optical depth is unity"""
        cdef int index_tau

        self.compute(["thermodynamics"])

        for index_tau in xrange(self.th.tt_size):
            # z grows with index_tau
            # exp_m_kappa = exp(-kappa) = exp(-optical_depth)
            exp_m_kappa = self.th.thermodynamics_table[index_tau*self.th.th_size
                                                       + self.th.index_th_exp_m_kappa]

            if exp_m_kappa - np.exp(-1) < 0:
                if np.abs(exp_m_kappa - np.exp(-1)) < np.abs(old_exp_m_kappa - np.exp(-1)):
                    index_unity = index_tau
                else:
                    index_unity = index_tau - 1
                break

            old_exp_m_kappa = exp_m_kappa

        return self.th.z_table[index_unity]
    ...
    def get_current_derived_parameters(self, names):
        ...
        for name in names:
            if name == 'h':
                ...
                value = self.alphaM(0)
            elif 'G_light_smg_' in name:
                z = float(name.split('_')[-1])
                value = self.G_light_smg(z)
            elif 'G_matter_smg_' in name:
                z = float(name.split('_')[-1])
                value = self.G_matter_smg(z)
            else:
    ...
    cdef __MontePython_ask_for_array_component(self, name, double * carray, carray_size):
        ...
        try:
            ...
            #TODO: don't print unless verbose
