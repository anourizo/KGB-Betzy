...
int input_read_parameters(
  ...
  */
  ...
  if (pba->Omega0_smg != 0.) {
     ...
     */
    int has_tuning_index_smg = _FALSE_, has_dxdy_guess_smg = _FALSE_;
...
//           }
      ...
      if (strcmp(string1,"k_essence_scherrer") == 0) {
  	pba->gravity_model_smg = k_essence_scherrer;
  	pba->field_evolution_smg = _TRUE_;
  	flag2=_TRUE_;
    ...
    }//end of k_essence_scherrer

    if (strcmp(string1,"k_essence_power") == 0) {
  pba->gravity_model_smg = k_essence_power;
  pba->field_evolution_smg = _TRUE_;
  flag2=_TRUE_;
  ...
  class_read_list_of_doubles("parameters_smg",pba->parameters_smg,pba->parameters_size_smg);
	...
	*/
    ...
    pba->parameters_size_smg = 6;
    pba->is_conformally_coupled_smg = _TRUE_;
    pba->tuning_index_2_smg = 1; //default tuning index is scalar IC
    
    //TODO: parameters_smg[0] is xi_ini (in base model) or Omega_smg_0 (in submodel)
	...
	/*1) base model, user specifies everything!  */
	if (flag3==_FALSE_){
	  ...
	  class_read_list_of_doubles("parameters_smg",pba->parameters_smg,pba->parameters_size_smg);
        ...
        //TODO: for M2 tuning problems when beta = 0

	  /*  temporary allocation, will be rewritten latter
	   *  order is xi_ini, phi_ini, c2, c3, beta */
	  class_alloc(pba->parameters_smg, sizeof(double*)*pba->parameters_size_smg,pba->error_message);
	  double * input_params_cccg; //dummy allocation vector

	  double c2, c3, beta, M2_ini, xi_ini, V0;
      double Omega_smg_ini, xi_ini_3,a_ini;

	  /* normal branch
	    */
	  if (strcmp(string2,"normal_branch") == 0) {
          
        pba->gravity_submodel_smg = cccg_exp_normal;

        class_read_list_of_doubles("parameters_smg",input_params_cccg,3);
        
        beta = input_params_cccg[0];// C = exp(-2 beta phi)
        M2_ini = input_params_cccg[1];
        if (beta == 0){
            M2_ini = 1.;
        }
        
        //parameter is log10(Omega_smg_ini)
        Omega_smg_ini = pow(10,input_params_cccg[2]);
        
        V0 = 0; //cosmo constant contribution
	    c3 = -1.;
        c2 = -pow(216.*pba->Omega0_smg*pow(c3,2),1./3.);//TODO: ADD c3 dependence
        
        pba->scalar_back_branch_smg = 1;//TODO: pass the xi_ini instead
        
        //pass default tuning_indices
        if (has_tuning_index_smg == _FALSE_){
            pba->tuning_index_smg = 2; //use c3 for default tuning
            pba->tuning_dxdy_guess_smg = -72./pow(c2,2); // d(c2)/d(Omega_smg) = 2/xi^3
            has_tuning_index_smg = _TRUE_;
        }
        
    }//end of normal branch
    
    else if (strcmp(string2,"canonical") == 0) {
        
        pba->gravity_submodel_smg = cccg_exp_canonical;
        
        class_read_list_of_doubles("parameters_smg",input_params_cccg,4);
        ...
        if (flag1 == _TRUE_){
          if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
            input_params_cccg[3] = - pow(10, input_params_cccg[3]);
          }}
        ...
        //pass default tuning_indices
        if (has_tuning_index_smg == _FALSE_){
            V0 = pba->Omega0_smg; //value that works
            pba->tuning_index_smg = 5; //use V0 for default tuning
            pba->tuning_dxdy_guess_smg = 1.; // d(V0)/d(Omega_smg) = 1
            has_tuning_index_smg = _TRUE_;
        }
        
    }//end of canonical branch
    

    else {class_test(flag3 == _TRUE_,
		 errmsg,
		 "Galileon: you specified a gravity_submodel that could not be identified. \n Options are: normal_branch, canonical");
    };

	  
    
    /*  temporary allocation, will be rewritten latter
     *  order is xi, phi, c2, c3, beta  */
	  class_alloc(pba->parameters_smg, sizeof(double*)*6,pba->error_message);
	  double * input_params_gal; //dummy allocation vector

      
      /* Set parameters for submodels */
	  pba->parameters_smg[0] = Omega_smg_ini;
	  //TODO: add support for raw calculations
	  pba->parameters_smg[1] = M2_ini;
	  pba->parameters_smg[2] = c2;
	  pba->parameters_smg[3] = c3;
	  pba->parameters_smg[4] = beta;
      pba->parameters_smg[5] = V0;
      
      //TODO default tuning index is V0!
      if (has_tuning_index_smg == _FALSE_){
//       if (pba->parameters_smg[2] < 0){
//             pba->tuning_index_smg = 2; //use c3 for default tuning
//             pba->tuning_dxdy_guess_smg = -72./pow(pba->parameters_smg[2],2); // d(c2)/d(Omega_smg) = 2/xi^3
//           }
//       else{
            pba->tuning_index_smg = 5; //use V0 for default tuning
            pba->tuning_dxdy_guess_smg = 1.; // d(V0)/d(Omega_smg) = 1 
//           }
        ...
        class_test(has_dxdy_guess_smg == _TRUE_ && has_tuning_index_smg == _FALSE_,
		 errmsg,
            "Coupled Galileon: you gave dxdy_guess_smg but no tuning_index_smg. You need to give both if you want to tune the model yourself");
      
      }//end of submodels
      
//       class_read_int("scalar_branch_smg",pba->scalar_back_branch_smg);
    }//end of cccg_exp
    
    //Coupled galileons C = exp(beta * phi) with beta = beta_0 * f + beta_i (1 - f) with f = (1+tanh((phi-phiT)/Dphi))/2.
    if (strcmp(string1,"cccg_exp_tanh") == 0) {
	pba->field_evolution_smg = _TRUE_;
	flag2=_TRUE_; 
	pba->gravity_model_smg = cccg_exp_tanh;
    ...
    // ccg_exp_tanh
    //
    // beta_i = pba->parameters_smg[0];
    // beta_0 = pba->parameters_smg[1];
    // phiT = pba->parameters_smg[2];
    // Dphi = pba->parameters_smg[3];
    // M2_ini = pba->parameters_smg[4];
    // log10(Omega_smg_ini) = pba->parameters_smg[5];
    // c3 = pba->parameters_smg[6];
    // V0 = pba->parameters_smg[7];
    
    
	  class_read_list_of_doubles("parameters_smg",pba->parameters_smg,pba->parameters_size_smg);
    pba->parameters_smg[5] = pow(10, pba->parameters_smg[5]);

        if (has_tuning_index_smg == _FALSE_){
            // Tune V0
            pba->parameters_smg[7] = pba->Omega0_smg; //value that works 
            pba->tuning_index_smg = 7; //use V0 for default tuning
            pba->tuning_dxdy_guess_smg = 1.; // d(V0)/d(Omega_smg) = 1
            has_tuning_index_smg = _TRUE_;
        }
    }
...
if (strcmp(string1,"nkgb") == 0 || strcmp(string1,"n-kgb") == 0 || strcmp(string1,"N-KGB") == 0 || strcmp(string1,"nKGB") == 0) {
  ...
  class_test(pba->parameters_smg[2]<0.,errmsg,"In n-KGB, Rshift0>=0, or ICs for background can't be set.");
      ...
      class_test(flag2==_FALSE_,
		 errmsg,
		 "could not identify gravity_theory value, check that it is one of 'propto_omega', 'propto_scale', 'constant_alphas', 'eft_alphas_power_law', 'eft_gammas_power_law', 'eft_gammas_exponential', 'brans_dicke', 'galileon', 'nKGB', 'quintessence_monomial', 'quintessence_tracker', 'alpha_attractor_canonical', 'k_essence_scherrer' ...");
    ...
    if (pba->is_conformally_coupled_smg = _TRUE_){
        class_read_double("a_max_conformal_coupling_regulate_smg",pba->a_max_conformal_coupling_regulate_smg);
    }
...
//     }
    ...
    if (flag1==_TRUE_){
      ...
      class_test(pba->gravity_model_smg!=brans_dicke && pba->gravity_model_smg!=cccg_exp,
		 errmsg,
		 "You asked to tune M_pl(today) to %e but currently this is only allowed for Brans-Dicke and coupled galileon\n",
		 pba->M_pl_today_smg);
      
    if (pba->gravity_model_smg == cccg_exp){
        /* conformally coupled cubic galileon
         * parameters in raw model [log10(Omega_i), M2_i, c2, c3, beta]
         * tuning M2_0 naturally on M2_i (always) and if beta>0 (M2_i < M2_0)
        */
      class_read_int("tuning_index_2_smg",pba->tuning_index_2_smg);
      if (flag1 == _TRUE_){
       class_test(pba->tuning_index_2_smg != 4 && pba->tuning_index_2_smg != 1,
           errmsg,
           "Coupled Galileon: secondary tuning index =%i can only be 1 (phi_i) or 4 (beta)",pba->tuning_index_2_smg);
       
       //Tuning on beta
       if (pba->tuning_index_2_smg == 4){
           class_test(pba->parameters_smg[1]>pba->M_pl_today_smg,
                      errmsg,
                      "Coupled Galileon: secondary tuning on beta but M2_i = %g > M2_0 = %g requires negative beta, which is unstable! ",pba->parameters_smg[1],pba->M_pl_today_smg);
           //using approximate expression for initial guess
          class_read_int("initial_guess",pba->initial_guess);
           if (pba->initial_guess == 1){
             pba->parameters_smg[4] = 0.3*(pba->M_pl_today_smg-pba->parameters_smg[1])/0.05;
           }
           else if (pba->initial_guess == 2){ 
             if (pba->M_pl_today_smg == 1.){
               pba->parameters_smg[4] = log(pba->M_pl_today_smg + 0.1)/0.4;
             }
             else {
               pba->parameters_smg[4] = log(pba->M_pl_today_smg)/0.4;
             }
           }
           else if (pba->initial_guess == 3){ 
               // Integrating Eq. 45 of 2003.06396
               // \Delta t ~ 13 Gyr ~ 13 * 300 Mpc = 3900 Mpc
               // c3 = -1 = pba->parameters_smg[3]
               pba->parameters_smg[4] = pow(pow(pba->M_pl_today_smg-pba->parameters_smg[1], 2.) * (-6 * (-1)) / (pba->H0 * 3900), 1/3.);
             }
           else if (pba->initial_guess == 4){ 
               // Integrating alpha_M = \beta^2 because phi_0 seems to not depend on c3 
               // at least with log(Omega_smg_i) = -10
               pba->parameters_smg[4] = sqrt(-(log(pba->M_pl_today_smg) - log(pba->parameters_smg[1]))/log(ppr->a_ini_over_a_today_default));
             }
           else if (pba->initial_guess == 5){ 
               // Numerically I've seen log(Delta phi) = c0 + c1 * log(DeltaM2_smg)
               // with c0 ~ 0.6, c1 ~ 0.5 (varies slightly with c3)
               // This expression assumes that e^\beta phi ~ \beta phi
               pba->parameters_smg[4] = exp( log(pba->M_pl_today_smg-pba->parameters_smg[1]) * (1 - 0.5) - 0.6 );
             }
           else if (pba->initial_guess == 6){ 
               // Numerically I've seen log(Delta phi) = c0 + c1 * log(DeltaM2_smg)
               // with c0 ~ 0.6, c1 ~ 0.5 (varies slightly with c3)
               // This expression is exact
               double c0, c1;
               
               if (pba->parameters_smg[3] < -0.5){
                   c0 = -0.5;
                   c1 = 0.4;
               }
               else if (pba->parameters_smg[3] < -0.1){
                   c0 = -0.1;  // at c3=-0.1, c0~0.04
                   c1 = 0.44;
               }
               else if (pba->parameters_smg[3] < -0.01){
                   c0 = 0.3;  // at c3=-0.1, c0~0.04
                   c1 = 0.44;
               }
               else if (pba->parameters_smg[3] < -1e-3){
                   c0 = 0.53;
                   c1 = 0.48;
               }
               else {    //if (pba->parameters_smg[3] < -1e-6){
                   c0 = 0.8;
                   c1 = 0.5;
               }
               
               double DeltaPhi = exp( c0 + c1 * log(pba->M_pl_today_smg-pba->parameters_smg[1]));
               pba->parameters_smg[4] = log(pba->M_pl_today_smg/pba->parameters_smg[1]) / DeltaPhi;
           }
       }  
       
       
       //TODO: substitute error with actual values
//        class_test(pba->tuning_index_2_smg == 4 && pba->parameters_smg[4]==0,
//            errmsg,
//            "Coupled Galileon: you asked to tune with beta, but set beta=0 for which M*=1 is fixed automatically");
      }
    }//end of cccg_exp
    ...
    if (flag1 == _TRUE_){
      if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
		 ...
		 errmsg,
		 "You asked to tune G_eff(today) to %e but currently this is only allowed for Brans-Dicke\n",
		 pba->M_pl_today_smg);
...
//       printf("updating param = %e to tune M_pl \n",pba->parameters_smg[pba->tuning_index_2_smg]);
  ...
  /** rescale alphas? (multipy by rescale_alpha_x, e.g. to distinguish background from perturbation effects) */
  class_read_double("rescale_alpha_b_smg",pba->rescale_alpha_b_smg);
  class_read_double("rescale_alpha_m_smg",pba->rescale_alpha_m_smg);
...
int input_default_params(
  ...
  pba->gravity_submodel_smg = unspecified;
  ...
  pba->a_max_conformal_coupling_regulate_smg = 0; /** < never use conformal coupling regularization for a > a_max */
  pba->is_conformally_coupled_smg==_FALSE_; /* is the scalar field conformally coupled (G4=G4(phi), C5=0)? Equivalent to c_g = c*/
  ...
  pba->rescale_alpha_b_smg = 1;
  pba->rescale_alpha_m_smg = 1;
  ...
  pba->scalar_back_branch_smg = 1; /* choice of branch for initial conditions, whenever multiple attractors exist */
